...
public abstract class ParentBaseServiceImpl implements BaseService {
  ...
  public Mono<Object> findByIdFullHierarchy(String parentId) {
    ...
    return getParentRepository().findById(parentId)
        .map(entity -> getMapper().toModel(entity));
  }

  public Flux<Object> findAllFullHierarchy() {
    return getParentRepository().findAll()
        .map(baseEntity -> getMapper().toModel(baseEntity));
  }
  ...
  public Mono<Object> updateFullHierarchy(String parentId, Mono<Object> parentMono) {
    ...
    return findByIdFullHierarchy(parentId)
        ...
        .flatMap(existing -> parentMono.flatMap(parentObject -> {
          BaseEntity newEntity = getMapper().toEntity(parentObject, null, true);
          return getParentRepository().update(parentId, newEntity)
              .flatMap(rowsUpdated -> {
                if (rowsUpdated == null || rowsUpdated <= 0)
                  return Mono.error(new RuntimeException("Failed to update parent with id: " + parentId));
  ...
  public Mono<Long> deleteFullHierarchy(String parentId) {
    ...
    // Delete directly without going through mapper — avoids enum mapping errors
    return getParentRepository().deleteById(parentId)
        .as(transactionalOperator::transactional)
        .onErrorResume(ex ->
            Mono.error(new RuntimeException("Failed to delete: " + parentId, ex)));
  }
}